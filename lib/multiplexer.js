...
util.extend(Multiplexer, {
    ...
    setup: function (config) {
        if (!(config && config.replication && config.replication)) { return; }
        ...
        replConfig = util.fill(config.replication, DEFAULT_REPL_SOURCE_CFG); // extract repl config from main config
        ...
        peerBaseConfig = {
            enabled: true,
            database: config.database, // force db name to be same
            pool: true,
        ...
        if (peerNames.length === 0) {
            ...
            (replConfig.inheritMaster === true) && (this.source = db.createSource(util.extend(config, peerBaseConfig)));
            return;
        }
        ...
        if (peerNames.length === 1) {
            ...
            // from master
            this.source = db.createSource(util.fill(util.extend(replConfig.sources[peerNames.pop()], peerBaseConfig),
                (replConfig.inheritMaster === true) && config)); // create source and bugger off!
            return;
        }

        // iterate over all sources and normalise and validate their configuration
        util.each(replConfig.sources, function (peerConfig) {
            // set defaults or inherit from master
            util.fill(util.extend(peerConfig, peerBaseConfig), (replConfig.inheritMaster === true) && config);
        });
