var nonObjectCriteriaTypes = { // object to validate types
    ...
    PRIMARY_CONNECTION_ID = util.uid(),
 ...
 *
 * @constructor
 * @param {string=} [connectionName=primary] - optionally provide a connection name to start this stransaction with
 * @param {function} callback - receives `error`, `transaction`
 */
Transaction = function (connectionName, callback) {
    var connection = null,
        source = Transaction.sources[connectionName];

    // validate db setup prior to every connection. this ensures nothing goes forward post teardown or pre setup
    if (!source) {
    ...
    util.extend(this, /** @lends Transaction.prototype */ {
        ...
        connect: function (callback) {
            if (!connection) {
                source.getConnection(function (error, conn) {
                    if (!error) {
...
util.extend(Transaction, /** @lends Transaction */ {
    ...
    setup: function (config) {
        ...
        if (this.sources[PRIMARY_CONNECTION_ID]) {
        ...
        if (this.sources[PRIMARY_CONNECTION_ID]) {
            // @todo - emit wrror event instead of console.log
            console.log('Warn: replica set named "%s" will be overridden by main source. Rename it.',
                PRIMARY_CONNECTION_ID);
        }

        // create the primary db
        this.sources[PRIMARY_CONNECTION_ID] = db.createSource(config);
    },
    ...
    start: function (callback) {
        return (new this(PRIMARY_CONNECTION_ID, callback));
    },

    /**
     * Start a new transaction using a particular connection
     *
     * @param {function} callback - receives (`error`, `{Transaction#}`)
     */
    startUsing: function (connectionName, callback) {
        return (new this(connectionName, callback));
    },
