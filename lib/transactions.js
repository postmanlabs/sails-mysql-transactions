...
Transaction = function (callback) {
    ...
    util.extend(this, /** @lends Transaction.prototype */ {
        ...
        connect: function (connectionName, callback) {
            ...
            if (!connection) {
                // if transaction id is not set for the instance, set one.
                if (!self._id) {
                    self._id = util.uid();
                }
                ...
                _db.getConnection(function (error, conn) {
                    if (!error) {
                        ...
                        connection._transactionConfig = _db.transactionConfig;
                        Transaction.associateConnection(conn, self._id); // assign a transaction id
                    }
...
util.extend(Transaction, /** @lends Transaction */ {
     ...
     * @param {string}           uid
     */
    associateConnection: function (connection, uid) {
...
util.extend(Transaction.prototype, /** @lends Transaction.prototype */ {
     ...
     */
    wrap: function (query) {
        return util.wrapquery(query, this._id);
    },
    ...
    commit: function (callback) {
        ...
        conn ? conn.commit(function (error) {
            ...
            if (error && conn._transactionConfig.rollbackOnError) {
