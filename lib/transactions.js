...
util.extend(Transaction.prototype, /** @lends Transaction.prototype */ {
  begin: function (callback) {
    ...
    if (!conn) {
      this.connect(function (error, conn) {
        ...
        // connection branch. it is always highly likely that this would be the program flow. in a very unlikely case
        // the alternate flow will kick in, which is the `conn.query` right after this if-block.
        conn.query(SQL_START, function (error) {
          callback(error, self);
        });
      });
  ...
  act: function (query) {
    ...
    // @todo - emit error event instead of console.log
    else {
      console.log('Warn: Transaction was executed without doing Transaction.begin()');
    }
  ...
  toString: function () {
    return '[object Transaction: ' + (this.id() || 'disconnected') + ']';
  },
 ...
 */
Transaction.retrieveConnection = function (id) {
 ...
 */
Transaction.setup = function (config) {
...
Transaction.teardown = function () {
  ...
  // now execute end on the db. will end pool if pool, or otherwise will execute whatever `end` that has been exposed
  // by db.js
  if (this.db) {
    this.db.end();
    this.db = null;
 ...
 * @returns {string}
 */
Transaction.extractIdFromCriteria = function (criteria) {
  ...
  if (criteria.transactionId) {
    // @todo this is brought here because there is an extra transactionId coming from somewhere in the where clause
    if (criteria.where && criteria.where.transactionId) {
      delete criteria.where.transactionId;
    }

    // if the transaction id is active, then return the same
    if (conns[transactionId]) {
  ...
  if (Array.isArray(criteria)) {
    ...
    // if transaction id was found in recursive criteria, we return the same, else proceed.
    if (transactionId) {
  ...
  if (criteria.where && criteria.where.transactionId) {
    // if transaction id was found in recursive criteria, we return the same, else proceed.
    if (conns[criteria.where.transactionId]) {
      return ((transactionId = criteria.where.transactionId), (delete criteria.where.transactionId), transactionId);
    }
    delete criteria.where.transactionId;
  }
