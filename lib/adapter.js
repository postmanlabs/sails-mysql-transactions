...
util.each({
    ...
    adapter[prop] = function (connectionName, collectionName) {
        ...
        var args = arrProtoSlice.call(arguments),
            firstCustomArg = args[override.conn + 1],
            id;

        // test for multiplexed command
        if (Multiplexer.retrieveConnection(firstCustomArg)) {
            args[override.conn] = Multiplexer.retrieveConnection(firstCustomArg);
        }
        else {
            if (override.hasOwnProperty('probe')) {
                ...
                id = Transaction.probeIdFromValues(firstCustomArg);
            }
...
util.extend(adapter, {
     ...
     */
    readonly: function (connectionName, collectionName, sourceName) {
        return (new Multiplexer(this, sourceName));
    }
});
